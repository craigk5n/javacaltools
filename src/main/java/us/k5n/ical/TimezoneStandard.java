...
/**
 * iCalendar Timezone STANDARD sub-component.
 * The STANDARD sub-component defines the standard time observance within a VTIMEZONE component.
 * 
 * @author Craig Knudsen, craig@k5n.us
 */
public class TimezoneStandard implements Constants {
	...
	/**
	 * Create a TimezoneStandard object based on specified iCalendar data
	 * 
	 * @param parser
	 ...
	 */
	public TimezoneStandard(CalendarParser parser, int initialLine, List<String> textLines) {
	...
	public void parseLine(String icalStr, int parseMethod)
		...
		if (up.equals("BEGIN:STANDARD") || up.equals("END:STANDARD")) {
			...
			throw new ParseException("Unrecognized data in STANDARD: " + icalStr, icalStr);
		}
	}
	...
	/**
	 * Was enough information parsed for this TimezoneStandard to be valid?
	 * 
	...
	/**
	 * Check if this TimezoneStandard is valid with optional error details
	 *
	 * @param errors List to collect validation error messages (can be null)
	 * @return true if the timezone standard is valid
	 */
	public boolean isValid(List<String> errors) {
		...
		if (dtstart == null) {
			valid = false;
			if (errors != null) errors.add("TimezoneStandard must have DTSTART");
		}

		if (tzOffsetFrom == null) {
			valid = false;
			if (errors != null) errors.add("TimezoneStandard must have TZOFFSETFROM");
		}

		if (tzOffsetTo == null) {
			valid = false;
			if (errors != null) errors.add("TimezoneStandard must have TZOFFSETTO");
		}

		// TZOFFSET validation
		if (tzOffsetFrom != null && !isValidTimezoneOffset(tzOffsetFrom)) {
			valid = false;
			if (errors != null) errors.add("TimezoneStandard TZOFFSETFROM is invalid: " + tzOffsetFrom);
		}

		if (tzOffsetTo != null && !isValidTimezoneOffset(tzOffsetTo)) {
			valid = false;
			if (errors != null) errors.add("TimezoneStandard TZOFFSETTO is invalid: " + tzOffsetTo);
		}

		// RRULE validation for recurrence (basic check)
		if (rrule != null && rrule.value == null) {
			valid = false;
			if (errors != null) errors.add("TimezoneStandard RRULE is malformed");
		}
	...
	/**
	 * Convert this TimezoneStandard to iCalendar format
	 * 
	 * @return iCalendar string representation
	 */
	public String toICalendar() {
		StringBuffer ret = new StringBuffer(1024);
		ret.append("BEGIN:STANDARD");
		ret.append(CRLF);
		...
		ret.append("END:STANDARD");
		return ret.toString();
	}

	/**
	 * Get a string representation of this TimezoneStandard
	 * 
	 * @return string representation
	 */
	public String toString() {
		return "TimezoneStandard [dtstart=" + dtstart + 
