...
public class StringUtils implements Constants {
	...
	/**
	 * Format a string into ical folded-line format (which states that lines should
	 * not be longer than 75 characters, excluding the line break.)
	 * 
	 * @param x The input string to be formatted
	 ...
	 * 
	 * @param line Input iCalendar line
	...
	/**
	 * Unfold a multi-line iCalendar String into a single line. New line escape
	 * sequences will be converted back to CRLF (015 012). If a line is longer than
	 * 75 characters, a ParseException will be generated if parseMode is set to
	 * PARSE_STRICT. Additionally, non-ASCII characters will be converted from their
	 * quoted-printable format. If parseMode is not set to PARSE_STRICT, escaped
	 * commas "\," will be unescaped. (Mozilla Sunbird does this escaping which is
	 * not part of the RFC 2445 specification.)
	 * 
	 * @param line      Input iCalendar line
	 * @param parseMode PARSE_STRICT or PARSE_LOOSE; PARSE_STRICT will generate an
	 *                  exception on lines too long.
	 * @return The "unfolded" line
	 * @see foldLine
	 */
	// TODO: convert quotable-printable format
	static public String unfoldLine(String line, int parseMode) throws ParseException {
		int len = 0;
		...
		for (int i = 0; i < line.length(); i++) {
			...
			if (len > MAX_LINE_LENGTH) {
				if (parseMode == PARSE_STRICT) {
					throw new ParseException("Found line longer than " + MAX_LINE_LENGTH + " limit", line);
				}
			}
			...
			if (ch == '\\' && ch2 == 'n') {
				...
				// the data line.
				if (parseMode == PARSE_STRICT) {
					throw new ParseException("Invalid line termination at char " + i, line);
				} else {
	 ...
	 */
	static public int parseTransp(String icalStr, int parseMethod) throws BogusDataException, ParseException {
		Property p = new Property(icalStr, parseMethod);
		if (icalStr == null || icalStr.trim().length() == 0 || p == null || p.getValue() == null) {
	 ...
	 */
	static public int parseStatus(String icalStr, int parseMethod) throws BogusDataException, ParseException {
		Property p = new Property(icalStr, parseMethod);
		if (icalStr == null || icalStr.trim().length() == 0 || p == null || p.getValue() == null) {
