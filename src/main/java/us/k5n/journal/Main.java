...
public class Main extends JFrame implements Constants, RepositoryChangeListener {
	...
	JToolBar createToolBar() {
		...
		deleteButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				if (ind >= 0 && filteredSearchedJournalEntries != null
						&& ind < filteredSearchedJournalEntries.size()) {
					Journal j = (Journal) filteredSearchedJournalEntries.get(ind);
	...
	void updateDateTree() {
		...
		for (int i = years.length - 1; years != null && i >= 0; i--) {
			List<Journal> yearEntries = dataRepository.getEntriesByYear(years[i]);
			...
			for (int j = 0; months != null && j < months.length; j++) {
				List<Journal> monthEntries = dataRepository.getEntriesByMonth(years[i],
						months[j]);
	...
	private void export(String title, List<Journal> journalEntries) {
