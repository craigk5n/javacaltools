...
/**
  * iCal StringFormatter utility class
  * @version $Id$
  ...
  implements Constants
{
  ...
  /**
    * Format a string into ical folded-line format (which states
    * that lines should not be longer than 75 characters, excluding
    * the line break.)
  ...
  /**
    * Unfold a multi-line iCal String into a single line.
    * New line escape sequences will be converted back to
    * CRLF (015 012).  Rather than throwing an ParseException
    * when lines longer than 75 characters are encountered, the
    * error will be ignored for greater compatibility.
    * Additionally, non-ASCII characters will be converted from
    * their quoted-printable format.
    * @param line	Input iCal line
    * @return		The "unfolded" line
    * @see foldLine
    */
  static public String unfoldLine ( String line )
  ...
  /**
    ...
    * New line escape sequences will be converted back to
    * CRLF (015 012).
    * If a line is longer than 75 characters, a ParseException will
    * be generated if parseMode is set to PARSE_STRICT.
    * Additionally, non-ASCII characters will be converted from
    * their quoted-printable format.
    * If parseMode is not set to PARSE_STRICT, escaped commas "\," will
    * be unescaped.  (Mozilla Sunbird does this escaping which is not
    * part of the RFC 2445 specification.)
    * @param line	Input iCal line
    * @param parseMode	PARSE_STRICT or PARSE_LOOSE; PARSE_STRICT will
    *			generate an exception on lines too long.
    * @return		The "unfolded" line
    * @see foldLine
    */
    ...
    throws ParseException
  {
    ...
    for ( int i = 0; i < line.length(); i++ ) {
      ...
      if ( ch == '\\' && ch2 == 'n' && ch == '=' && parseMode == PARSE_LOOSE ) {
        ...
        // the data line.
        if ( parseMode == PARSE_STRICT ) {
          throw new ParseException ( "Invalid line termination at char " +
            i, line );
        } else {
